    ...
    <div className="landing-page">
      ...
      <nav className="lp-navbar">
        ...
        <div className="lp-nav-links">
          {['Home', 'Destination', 'About Us', 'Tour', 'Testimonial', 'Blog'].map((link) => (
            <span 
              key={link} 
              className="lp-nav-link"
              onClick={() => setActiveTab(link)}
              style={{ color: activeTab === link ? 'var(--lp-primary)' : '#0f172a' }}
            >
              {link}
            </span>
          ))}
      ...
      <section style={{ padding: '80px 6%', background: 'var(--lp-bg-alt)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '40px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button style={{ width: '40px', height: '40px', borderRadius: '50%', border: '1px solid #cbd5e1', background: 'white', display: 'flex', alignItems: 'center', justifyX: 'center', justifyContent: 'center', cursor: 'pointer' }}>
              <ChevronLeft size={16} />
            </button>
